describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let prodShadow = await prodItem.getProperty("shadowRoot");
    let prodButton = await prodShadow.$("button");
    await prodButton.click();
    let prodText = await prodButton.getProperty("innerText");
    let changed = true;
    if(prodText["_remoteObject"].value !== "Remove from Cart") {
      changed = false;
    }
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(let i = 0; i < prodItems.length; i++) {
      ...
      if(prodText["_remoteObject"].value !== "Add to Cart") {
        allAreRemoved = false;
      }
    }
